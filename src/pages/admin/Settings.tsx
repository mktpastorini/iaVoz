...
const settingsSchema = z.object({
  ...
  ai_model: z.enum(["gpt-4-turbo", "gpt-3.5-turbo", "gpt-4o-mini", "gemini-2.5-pro-001", "gemini-2.5-flash-001", "gemini-pro", "gemini-flash"]),
  ...
  gemini_tts_voice_name: z.string().optional().nullable(),
...
const GEMINI_TTS_MODELS = [
    { value: "gemini-2.5-flash-preview-tts", label: "Gemini 2.5 Flash Preview" },
    { value: "gemini-2.5-pro-preview-tts", label: "Gemini 2.5 Pro Preview" },
];
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><Label>Modelo de IA (Cérebro)</Label><Controller control={control} name="ai_model" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="gpt-4o-mini">OpenAI - GPT-4o Mini</SelectItem><SelectItem value="gpt-4-turbo">OpenAI - GPT-4 Turbo</SelectItem><SelectItem value="gpt-3.5-turbo">OpenAI - GPT-3.5 Turbo</SelectItem><SelectItem value="gemini-2.5-pro-001">Vertex AI - Gemini 2.5 Pro</SelectItem><SelectItem value="gemini-2.5-flash-001">Vertex AI - Gemini 2.5 Flash</SelectItem><SelectItem value="gemini-pro">Google Gemini - Pro (Legado)</SelectItem><SelectItem value="gemini-flash">Google Gemini - Flash (Legado)</SelectItem></SelectContent></Select>)} /></div>
          ...
          {voiceModel === 'gemini-tts' && <><div><Label>Modelo Gemini TTS</Label><Controller control={control} name="gemini_tts_model" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{GEMINI_TTS_MODELS.map(v => <SelectItem key={v.value} value={v.value}>{v.label}</SelectItem>)}</SelectContent></Select>)} /></div><div><Label>Voz Gemini TTS</Label><Controller control={control} name="gemini_tts_voice_name" render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{GEMINI_TTS_VOICES.map(v => <SelectItem key={v.value} value={v.value}>{v.label}</SelectItem>)}</SelectContent></Select>)} /></div></>}
        </CardContent>
      </Card>
