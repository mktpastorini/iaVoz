...
// Main Component
const FuturisticVoiceAssistant: React.FC<VoiceAssistantProps> = ({
  settings,
  isLoading,
}) => {
  ...
  const { audioIntensity, isSpeaking: isAudioPlaying } = useAssistantAudio({ audioElementRef: audioRef });
  ...
  const speak = useCallback(async (text: string, onEndCallback?: () => void) => {
    ...
    try {
      if (currentSettings.voice_model === "browser" && synthRef.current) {
        ...
        utterance.onerror = (e) => { console.error('[VA] Erro na síntese de voz:', e); onSpeechEnd(); };
  ...
  const runConversation = useCallback(async (userInput: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Erro na API OpenAI: ${response.statusText}`);
      ...
      if (responseMessage.tool_calls) {
        ...
        const toolOutputs = await Promise.all(responseMessage.tool_calls.map(async (toolCall: any) => {
          ...
          if (isInternalFunction && functionName) {
            ...
            const processedUrl = replacePlaceholders(power.url || '', allVars);
            const processedHeaders = power.headers ? JSON.parse(replacePlaceholders(JSON.stringify(power.headers), allVars)) : {};
            const processedBody = (power.body && ["POST", "PUT", "PATCH"].includes(power.method)) ? JSON.parse(replacePlaceholders(JSON.stringify(power.body), allVars)) : undefined;
        ...
        if (!secondResponse.ok) throw new Error(`Erro na 2ª chamada OpenAI: ${secondResponse.statusText}`);
      ...
      console.error('[VA] Erro na conversa:', error);
  ...
  const initializeAssistant = useCallback(() => {
    ...
    recognitionRef.current.onerror = (e) => { if (e.error === 'not-allowed') setMicPermission('denied'); };
    recognitionRef.current.onresult = (event) => {
      ...
      if (isOpenRef.current) {
        ...
        if (settingsRef.current && transcript.includes(settingsRef.current.activation_phrase.toLowerCase())) {
          setIsOpen(true);
          const message = hasBeenActivatedRef.current ? settingsRef.current.continuation_phrase : settingsRef.current.welcome_message;
          speak(message);
  ...
  const checkAndRequestMicPermission = useCallback(async () => {
    try {
      ...
      permission.onchange = () => checkAndRequestMicPermission();
    } catch (error) {
      setMicPermission('denied');
    }
  ...
  }, [isOpen, handleManualActivation, stopSpeaking]);
  ...
  return (
    <>
      ...
      <div className={cn("fixed inset-0 z-50 transition-opacity duration-500", isOpen ? "opacity-100" : "opacity-0 pointer-events-none")}>
        <Canvas camera={{ position: [0, 0, 2] }}>
          <ambientLight intensity={0.5} />
          <CosmicBackground quality={quality} />
          <AiOrb audioIntensity={audioIntensity} isSpeaking={isAudioPlaying} quality={quality} />
        ...
        <AssistantUI
          ...
          onToggleMic={toggleAssistantOpen}
        />
      </div>
      ...
      {micPermission === 'granted' && (
        <div className="fixed bottom-4 right-4 z-[1000]">
          <Button
            ...
            size="lg"
            className={cn("rounded-full w-16 h-16 shadow-lg", isOpen ? "bg-red-600" : "bg-cyan-500", (isListening || isAudioPlaying) && "animate-pulse")}
          >
