...
// Layer 3: Cosmic Background - sutil e escuro
...
// Layer 2: Energy Lines - mais caóticos e expansivos
...
const EnergyLines = ({ count = 10, radius = 1.5 }) => {
  const lines = useMemo(() => {
    return Array.from({ length: count }, () => {
      // Pontos aleatórios que podem se estender além do raio para efeito de expansão
      const points = Array.from({ length: 12 }, (_, i) => {
        ...
        const scalar = i < 6 ? radius * (0.3 + Math.random() * 0.7) : radius * (1.5 + Math.random() * 1.5);
...
// Layer 1: Particle Orb com movimento fluido e nebuloso
const ParticleOrb = () => {
  ...
  const vertexShader = `
    ...
    // Simplex noise functions omitted for brevity, using classic noise approximation
    float rand(vec2 co){
    ...
    void main() {
      ...
      // Movimento fluido simples baseado em seno e ruído randômico
      ...
      <div className={cn("fixed inset-0 z-[9999] flex flex-col items-center justify-center transition-opacity duration-500", isOpen ? "opacity-100" : "opacity-0 pointer-events-auto")}>
        ...
        <div className="relative z-20 flex flex-col items-center justify-between w-full h-full p-8 pointer-events-auto">
          ...
          <div className="text-center">
            {displayedAiResponse && (
              <div className="bg-black/30 backdrop-blur-sm border border-white/20 rounded-lg p-4 max-w-lg mx-auto select-text">
                <p className="text-white text-2xl md:text-4xl font-bold leading-tight drop-shadow-lg">{displayedAiResponse}</p>
              </div>
            )}
            {transcript && <p className="text-gray-400 text-lg mt-4 select-text">{transcript}</p>}
          </div>
