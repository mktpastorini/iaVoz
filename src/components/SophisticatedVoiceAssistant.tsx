  ...
  const runConversation = useCallback(async (userInput: string) => {
    ...
    try {
      ...
      if (responseMessage.tool_calls) {
        ...
        // **CORREÇÃO:** Adiciona a mensagem da IA à história, mantendo `content` como `null` se for o caso.
        ...
        const toolOutputs = await Promise.all(responseMessage.tool_calls.map(async (toolCall: any) => {
          ...
          // **CORREÇÃO:** A autenticação agora é tratada pela `proxy-api`, então não é mais necessário adicionar cabeçalhos aqui.
